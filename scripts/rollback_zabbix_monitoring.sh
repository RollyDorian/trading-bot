...
systemctl disable --now hibachi-collect-failure-retention.service >/dev/null 2>&1 || true
...
rm -f -- /etc/systemd/system/hibachi-collect-failure-retention.service
rm -f -- /etc/hibachi-collect-monitor.conf
rm -f -- /usr/local/libexec/hibachi-zabbix-cache
rm -f -- /usr/local/libexec/hibachi-collect-failure-retention
